#ifndef MXNET_OPERATOR_CONTRIB_FLK_CONVOLUTION_INL_H_
#define MXNET_OPERATOR_CONTRIB_FLK_CONVOLUTION_INL_H_
...
namespace conv {
  enum FixLengthKernelConvolutionOpInputs { kData, kKernelMasks, kWeight, kBias };
  enum FixLengthKernelConvolutionOpOutputs { kOut };
  enum FixLengthKernelConvolutionOpResource { kTempSpace };
}
struct FixLengthKernelConvolutionParam : public dmlc::Parameter<FixLengthKernelConvolutionParam> {
  TShape kernel;
  TShape stride;
  TShape dilate;
  TShape pad;
  ...
  DMLC_DECLARE_PARAMETER(FixLengthKernelConvolutionParam) {
...
class FixLengthKernelConvolutionOp : public Operator {
 public:
  explicit FixLengthKernelConvolutionOp(FixLengthKernelConvolutionParam p) {
    this->param_ = p;
  ...
  virtual void Forward(const OpContext &ctx,
    ...
    col_buffer_shape[0] = conv_out_channels_ * conv_in_channels_ * param_.kernel.Size();
    ...
    TBlob output_buffer(workspace.dptr_, col_buffer_shape, xpu::kDevMask, DataType<DType>::kFlag);
	...
	Tensor<xpu, 3, DType> output_buffer_3d = output_buffer.get_with_shape<xpu, 3, DType>(
      Shape3(group_, M * K, N), s); // (g, Cout/g*Cin/g*kmax, H*W)
	Tensor<xpu, 3, DType> output_3d = in_data[conv::kOut].get_with_shape<xpu, 3, DType>(
      Shape3(num_, conv_out_channels_, N), s); // (B, Cout, H*W)

	const int NDim = 2; // for broadcasting
	TShape new_rshape(2);
	new_rshape[0] = M * K; new_rshape[1] = N;
	TShape new_lshape(2);
	new_lshape[0] = M * K; new_lshape[1] = 1;
	TShape new_oshape(2);
	new_oshape[0] = M * K; new_lshape[1] = N;
 ...
 private:
  FixLengthKernelConvolutionParam param_;
...
Operator* CreateOp(FixLengthKernelConvolutionParam param, int dtype,
  std::vector<TShape> *in_shape,
  std::vector<TShape> *out_shape,
  Context ctx);

#if DMLC_USE_CXX11
class FixLengthKernelConvolutionProp : public OperatorProperty {
 public:
  ...
  OperatorProperty* Copy() const override {
    auto ptr = new FixLengthKernelConvolutionProp();
    ptr->param_ = param_;
    return ptr;
  }

  std::string TypeString() const override {
    return "_contrib_FixLengthKernelConvolution";
  }
