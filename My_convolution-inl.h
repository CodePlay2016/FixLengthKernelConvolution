#ifndef MXNET_OPERATOR_CONTRIB_MY_CONVOLUTION_INL_H_
#define MXNET_OPERATOR_CONTRIB_MY_CONVOLUTION_INL_H_
...
namespace conv {
  enum MyConvolutionOpInputs { kData, kKernelMasks, kWeight, kBias };
  enum MyConvolutionOpOutputs { kOut };
  enum MyConvolutionOpResource { kTempSpace };
}
struct MyConvolutionParam : public dmlc::Parameter<MyConvolutionParam> {
  TShape kernel;
  TShape stride;
  TShape dilate;
  TShape pad;
  ...
  DMLC_DECLARE_PARAMETER(MyConvolutionParam) {
...
class MyConvolutionOp : public Operator {
 public:
  explicit MyConvolutionOp(MyConvolutionParam p) {
    this->param_ = p;
 ...
 private:
  void LayerSetUp(const TShape& ishape, const TShape& kernel_masks_shape, const TShape& oshape) {
    ...
    kernel_dim_ = conv_in_channels_ / group_ * param_.kernel.Size(); // kernel.Size():kh*kw
...
Operator* CreateOp(MyConvolutionParam param, int dtype,
  std::vector<TShape> *in_shape,
  std::vector<TShape> *out_shape,
  Context ctx);

#if DMLC_USE_CXX11
class MyConvolutionProp : public OperatorProperty {
 public:
  ...
  OperatorProperty* Copy() const override {
    auto ptr = new MyConvolutionProp();
    ptr->param_ = param_;
    return ptr;
  }

  std::string TypeString() const override {
    return "_contrib_MyConvolution";
  }
